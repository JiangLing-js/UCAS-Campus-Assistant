...
let seenNotifications=new Set(),loaded=false;
...
async function refresh({render=false}={}){
 ...
 if(loaded&&'Notification'in window&&Notification.permission==='granted')for(const n of state.notifications){if(!seenNotifications.has(n.id)&&!n.seen)new Notification(n.title,{body:n.body,icon:'/icon.svg',tag:n.id});}
 seenNotifications=new Set(state.notifications.map(n=>n.id));loaded=true;
...
function renderList(){
 ...
 $('#main-content').innerHTML=`<div class="page"><div class="page-heading"><div><h2>${names[view]}</h2><p>${config.desc}</p></div><button class="primary" data-action="add">＋ 添加事项</button></div><div class="filters">${config.filters.map(f=>`<button class="chip ${filter===f?'active':''}" data-filter="${f}">${({all:'全部',done:'已完成',past:'往期'})[f]||kinds[f]}</button>`).join('')}<input id="list-search" class="search" type="search" value="${escape(query)}" placeholder="搜索标题、地点或内容" aria-label="搜索事项"></div>${items.length?items.map(itemCard).join(''):empty('这里暂时没有事项。你可以添加一件事，或同步校园页面。')}<p class="footer-note">${items.length} 条事项 · 通知适用范围和讲座报名状态以学校原文为准。</p></div>`;
...
function addItem(){const form=$('#item-form');form.reset();form.elements.startsAt.value=today()+'T20:00';$('#item-error').textContent='';$('#item-dialog').showModal();form.elements.title.focus();}
$('#item-form').addEventListener('submit',async e=>{e.preventDefault();const data=Object.fromEntries(new FormData(e.target));data.startsAt=data.startsAt?new Date(data.startsAt+':00+08:00').toISOString():null;data.endsAt=data.endsAt?new Date(data.endsAt+':00+08:00').toISOString():null;data.remindMinutes=data.remindMinutes===''?null:Number(data.remindMinutes);try{await api('/items',data);$('#item-dialog').close();await refresh({render:true});toast('事项已保存');}catch(err){$('#item-error').textContent=err.message;}});
...
setInterval(()=>refresh({render:['notifications','calendar'].includes(view)}).catch(()=>{}),15000);
