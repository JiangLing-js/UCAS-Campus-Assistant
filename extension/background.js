...
chrome.runtime.onMessage.addListener((message,sender,respond)=>{
 ...
 if(message.type==='configure'){
  (async()=>{const server=localServer(message.server);if(!/^[a-f0-9]{48}$/.test(message.token||''))throw new Error('连接码格式不正确');const autoRemind=message.autoRemind===undefined?(await chrome.storage.local.get('autoRemind')).autoRemind===true:message.autoRemind===true;if(autoRemind&&!await chrome.permissions.contains({permissions:['notifications']}))throw new Error('请先允许通知权限');await chrome.storage.local.set({server,token:message.token,autoSync:!!message.autoSync,autoRemind,...(message.autoFullSync!==undefined?{autoFullSync:!!message.autoFullSync}:{})});await chrome.alarms.clear('campus-sync');if(message.autoSync)await chrome.alarms.create('campus-sync',{periodInMinutes:15});await restoreReminderAlarm();if(autoRemind)await deliverBackgroundReminders();await restoreSyncQueueAlarm();return message.syncAll?await startFullCampusSync():await sync();})().then(respond).catch(e=>respond({ok:false,message:e.message}));return true;
 }
});
