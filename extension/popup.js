...
chrome.storage.local.get(['server','token','autoSync','autoRemind','autoFullSync','lastFullSyncResult','lastResult','lastReminderResult']).then(c=>{
 if(c.server)$('server').value=c.server;if(c.token)$('token').value=c.token;
 $('auto').checked=!!c.autoSync;$('auto-full').checked=!!c.autoFullSync;$('reminders').checked=!!c.autoRemind;
 if(c.lastResult)$('status').textContent=c.lastResult.message;if(c.lastFullSyncResult)$('status').textContent=c.lastFullSyncResult.message;
...
$('pair').addEventListener('submit',async e=>{
 ...
 try{
  ...
  const r=await chrome.runtime.sendMessage({type:'configure',server:$('server').value.trim(),token:$('token').value.trim(),autoSync:$('auto').checked,autoFullSync:$('auto-full').checked,syncAll:true,autoRemind:$('reminders').checked});
