...
(() => {
 ...
 if(host==='sep.ucas.ac.cn'){
  ...
  if(modal&&visible(modal)){const systems=Array.from(modal.querySelectorAll('a')).filter(visible).map(a=>({name:a.innerText.trim().replace(/^New\s*/i,'').slice(0,150),url:clean(a.href)})).filter(s=>s.name&&/\/portal\/site\/\d+\/\d+$/.test(s.url));if(systems.length)return {...base,sourceUrl:'https://sep.ucas.ac.cn/sepCard/card',integration:'systems',systems};}
 }
