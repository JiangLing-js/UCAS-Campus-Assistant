...
export const integrationKinds=['mail','network','activities','systems'];
...
export const integrationSnapshot=z.object({sourceUrl:text.max(3000),capturedAt:z.string().datetime().optional(),integration:z.enum(integrationKinds),
 ...
 const host={mail:'mail.cstnet.cn',network:'portal.ucas.ac.cn',activities:'ek.ucas.edu.cn',systems:'sep.ucas.ac.cn'}[s.integration];
...
export function ingestIntegration(input){
 ...
 db.exec('BEGIN IMMEDIATE');try{
  ...
  recordSync(s.integration,'ok',`同步 ${count} 条；采集于 ${capturedAt}`);db.exec('COMMIT');
