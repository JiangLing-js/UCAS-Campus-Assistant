...
for(const entry of tracked){
 ...
 if(blockedPath.test(file)||privateRecord.test(file)||secretData.test(file)||(/(?:^|\/)\.env(?:\.|$)/.test(file)&&!file.endsWith('.env.example'))){fail(file,0,'private runtime/configuration/data file');continue;}
 if(!publicTypes.test(file)&&!['.gitignore','.gitattributes','.env.example','LICENSE'].includes(file)){fail(file,0,'file type needs explicit publication review');continue;}
 let bytes;
...
else console.log(`Public-file check passed: ${tracked.length} ${staged?'staged':'tracked'} text files; no blocked files or matching secret patterns.`);
