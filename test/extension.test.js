...
function background(initial={},tabs=[],frames=[],options={}){
 ...
 context.importScripts=file=>{if(file!=='login.js')vm.runInContext(fs.readFileSync(new URL('../extension/'+file,import.meta.url),'utf8'),context);};
 vm.runInContext(source,context);
 return {stored,requests,alarms,notices,deliver:()=>vm.runInContext('deliverBackgroundReminders()',context),restore:()=>vm.runInContext('restoreReminderAlarm()',context),send:message=>new Promise(resolve=>{for(const listener of listeners)listener(message,{id:'test-extension'},resolve);})};
}
